...
uhr1000 <- read.csv("../path/to/dataset.csv", 
                    header = TRUE, sep=",")
...
uhr1000 <- uhr1000[!is.na(uhr1000$sans_tot_0),]
...
vars <- c("caarms_DS_sev_0", "caarms_UTC_sev_0", "sans_tot_0", "transtat",
          "year")
...
# Run model
fit <- lrm(transtat ~ caarms_DS_sev_0 + caarms_UTC_sev_0 + sans_tot_0, 
           data = df, x=TRUE, y=TRUE)
# Internal validation
val.res <- validate(fit, method = "boot", B=1000)
# Correct for optimism
fit$coefficients <- fit$coefficients*val.res[4,5]
...
for(i in c(1:(nrow(df_val)-2))){
  ...
  dat <- rbind(dat_train, dat_val[1:i,])
  
  fit <- lrm(transtat ~ caarms_DS_sev_0 + caarms_UTC_sev_0 + sans_tot_0, 
             data = dat, x=TRUE, y=TRUE)

  val.res <- validate(fit, method = "boot", B=10)
...
x <- dat_f[c("caarms_DS_sev_0", "caarms_UTC_sev_0", "sans_tot_0")]
x <- as.matrix(x)
y <- dat_f$transtat
mmat<-matrix(c(1,1,1),1,3,byrow=TRUE)
...
dm <- matrix(betas_dm,nrow = dim(dat_f)[1],ncol = 4)

# drop last coef obs
dm[450,] <- dm[451,]

dm_val <- dm[(dim(dm)[1]-dim(df_val)[1]+1):(dim(dm)[1]),1:4]
...
write.csv(df_plot, ".../path/results.csv", col.names = TRUE)
...
dca.plot <- ggplot(dca.all, aes(x = threshold, y = net_benefit, color = variable)) +
  ...
  scale_y_continuous(name="Net benefit", breaks = c(0.00, 0.025, 0.05, 0.075), limits=c(-0.01, 0.075)) +
...
dat_f$year2 <- dat_f$year
dat_f[dat_f$year < 2015, "year2" ] <- 2014

dat_f$year2 <- as.factor(dat_f$year2)

table_desc <- dat_f %>% 
  dplyr::select(year2, assessment_age, gender, timeSxService, uhr_group, transtat, group, caarms_DS_sev_0, caarms_UTC_sev_0, sans_tot_0) %>% # keep only columns of interest
  mutate(
  ...
  tbl_summary(  
    ...
    type   = list(c(gender, group, transtat) ~ "dichotomous",
                  c(assessment_age, timeSxService, caarms_DS_sev_0, caarms_UTC_sev_0, sans_tot_0) ~ "continuous",
...
table_desc %>%
  as_flex_table() %>%
  flextable::save_as_docx(path = ".../path/descriptive_table.docx")